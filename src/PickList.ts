...
import { commands, ConfigurationTarget, Disposable, env, extensions, InputBoxOptions, QuickPick, QuickPickItemKind, Uri, window, workspace, WorkspaceConfiguration } from "vscode";
...
export class PickList {
    ...
    private constructor(config: WorkspaceConfiguration, pickList?: QuickPick<ImgItem>) {
        ...
        if (pickList) {
            ...
            this.quickPick.onDidAccept((e: any) => this.listChange(this.quickPick.selectedItems[0].imageType, this.quickPick.selectedItems[0].path));
    ...
    private showInputBox(type: number) {
        ...
        window.showInputBox(option).then((value) => {
            ...
            // set配置
            let keyArr = ["", "imagePath", "opacity", "blur", "", "", "", "", "", "", "backgroundCoverParticleOpacity", "backgroundCoverParticleColor", "backgroundCoverParticleCount"];
